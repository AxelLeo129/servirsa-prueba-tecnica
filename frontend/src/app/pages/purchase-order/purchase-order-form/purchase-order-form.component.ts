...
export class PurchaseOrderFormComponent implements OnInit {
  ...
  save(): void {
    ...
    if (this.isEditing && this.id) {
      this.generalService.put('/purchase-orders', this.id.toString(), data).then(() => {
        this.router.navigate(['/purchase-orders/' +  this.projectId + "/" + this.bdId]);
      });
    } else {
      data['project_id'] = this.projectId;
      data['budget_item_id'] = this.bdId;
      this.generalService.post('/purchase-orders', data).then(() => {
        this.router.navigate(['/purchase-orders/' +  this.projectId + "/" + this.bdId]);
      });
    }
  }
